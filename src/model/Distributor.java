...
public class Distributor {
  ...
  private void build() {
    ...
    int i = 0;
    while (it.hasNext()) {
      EntryCell entry_cell = it.next();
      initial_cars_on_road[i] = Solver.countInitialCars(entry_cell);
      entries[i] = entry_cell;
      sinks[i] = entry_cell.cell.getSink();
    ...
    for (int i = 0; i < nb_out_links; i++) {
      ...
      if (step == 0)
        ...
        cumulative_left_cars[i][step] =
            nb_cars + cumulative_left_cars[i][step - 1];
  ...
  /**
   * @return True if the split ratios gives an error-approximation of a Nash
   *         Equilibrium
   */
  ...
  public void findOptimalSplitRatio(int step, double error) {
    findOptimalSplitRatio(step, error, false);
  }

  public void findOptimalSplitRatio(int step, double error, boolean debug) {
    if (!is_built) {
    ...
    do {
      ...
      attempts++;
    } while (!isOptimal(TT, step, error) && attempts <= max_attempts);
    ...
    if (attempts >= max_attempts) {
      System.out
          .println("Fail to optimize after the maximum number of iteration ( "
              + max_attempts
              + ")");
    } else {
      System.out.println("Sucess after " + attempts + " iterations");
    }
    for (int i = 0; i < nb_out_links; i++) {
